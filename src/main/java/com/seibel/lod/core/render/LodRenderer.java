...
public class LodRenderer {
  ...
  /**
   * If true the LODs colors will be replaced with a checkerboard, this can be
   * used for debugging.
   */
  public DebugMode previousDebugMode = DebugMode.OFF;

  // This tells us if the renderer is enabled or not. If in a world, it should be
  // enabled.
  ...
  /**
   * if this is true the LOD buffers should be regenerated, provided they aren't
  ...
  /**
   * This HashSet contains every chunk that Vanilla Minecraft is going to render
   */
  ...
  /**
   * Besides drawing the LODs this method also starts the async process of
   * generating the Buffers that hold those LODs.
   *
   * @param lodDim               The dimension to draw, if null doesn't replace
   *                             the current dimension.
   * @param baseModelViewMatrix  This matrix stack should come straight from MC's
   *                             renderChunkLayer (or future equivalent) method
   * @param baseProjectionMatrix
   * @param partialTicks         how far into the current tick this method was
   *                             called.
   */
  public void drawLODs(LodDimension lodDim, Mat4f baseModelViewMatrix, Mat4f baseProjectionMatrix, float partialTicks,
    ...
    // TODO move the buffer regeneration logic into its own class (probably called
    // in the client api instead)
    // starting here...
    ...
    // FIXME: Currently, we check for last Lod Dimension so that we can trigger a
    // cleanup() if dimension has changed
    // The better thing to do is to call cleanup() on leaving dimensions in the
    // EventApi, but only for client-side.
    if (markToCleanup) {
    ...
    if (partialRegen || fullRegen) {
      // TODO MC.getPlayerBlockPos().getY() is wrong when portal nearby
      // ClientApi.logToChat(Level.INFO, "MC player pos: Y: " +
      // MC.getPlayerBlockPos().getY());
      // ClientApi.logToChat(Level.INFO, "MC dimension: " +
      // MC.getCurrentDimension().getDimensionName());
    ...
    // GL32.glBindFramebuffer(GL32.GL_FRAMEBUFFER,
    // MC_RENDER.getTargetFrameBuffer());
    GL32.glViewport(0, 0, MC_RENDER.getTargetFrameBufferViewportWidth(),
    ...
    if (MC.getWrappedClientWorld().getDimensionType().hasCeiling())
      farPlaneBlockDistance = Math.min(CONFIG.client().graphics().quality().getLodChunkRenderDistance(),
          LodUtil.CEILED_DIMENSION_MAX_RENDER_DISTANCE) * LodUtil.CHUNK_WIDTH;
    else
    ...
    Mat4f combinedMatrix = createCombinedMatrix(baseProjectionMatrix, baseModelViewMatrix, vanillaBlockRenderedDistance,
        farPlaneBlockDistance, partialTicks);
    ...
    shaderProgram.fillUniformData(combinedMatrix,
        MC_RENDER.isFogStateSpecial() ? getSpecialFogColor(partialTicks) : getFogColor(partialTicks), 0,
        MC.getWrappedClientWorld().getHeight(), MC.getWrappedClientWorld().getMinHeight(), farPlaneBlockDistance,
        vanillaBlockRenderedDistance, MC_RENDER.isFogStateSpecial());

    // Note: Since lightmapTexture is changing every frame, it's faster to recreate
    // it than to reuse the old one.
    ...
    // lightmapTexture.fillData(MC_RENDER.getLightmapTextureWidth(),
    // MC_RENDER.getLightmapTextureHeight(), MC_RENDER.getLightmapPixels());
      ...
      for (int i = 0; i < maxI; i++) {
        if ((-halfLen <= ox) && (ox <= halfLen) && (-halfLen <= oy) && (oy <= halfLen)) {
          ...
          int regionZ = oy + pos.y;
          {
            ...
            if (region == null)
              continue;
            if (region.render(lodDim, cameraPos, cameraBlockPos, cameraDir, !cullingDisabled, shaderProgram))
              drawCount++;
          }
        }
  ...
  private static float calculateNearClipPlane(float distance, float partialTicks) {
    ...
    return (float) (distance
        / Math.sqrt(1d + LodUtil.pow2(Math.tan(fov / 180d * Math.PI / 2d)) * (LodUtil.pow2(aspectRatio) + 1d)));
  }

  /**
   * create and return a new projection matrix based on MC's projection matrix
   *
   * @param projMat                      this is Minecraft's current projection
   *                                     matrix
  ...
  private static Mat4f createCombinedMatrix(Mat4f projMat, Mat4f modelMat, float vanillaBlockRenderedDistance,
    ...
    lodProj.setClipPlanes(calculateNearClipPlane(nearClipPlane, partialTicks),
  ...
  /**
   * cleanup and free all render objects. REQUIRES to be in render thread (Many
   * objects are Native, outside of JVM, and need manual cleanup)
   */
  ...
  /**
   * If this is called then the next time "drawLODs" is called the LODs will be
   * regenerated; the same as if the player moved.
   */
  ...
  private boolean updateVanillaRenderedChunks(LodDimension lodDim) {
    ...
    PosArrayGridList<BoolType> grid = new PosArrayGridList<>(edgeGrid.gridSize, edgeGrid.getOffsetX(),
        edgeGrid.getOffsetY());
  ...
  private void updateRegenStatus(LodDimension lodDim, float partialTicks) {
    ...
    if (ApiShared.previousLodRenderDistance != CONFIG.client().graphics().quality().getLodChunkRenderDistance()
      ...
      tryFullGen = true;
    } else if (CONFIG.client().advanced().debugging().getDebugMode() != previousDebugMode) { // did the user change
                                                                                             // the debug
                                                                                             // setting?
    ...
    if (newTime - prevPlayerPosTime > CONFIG.client().advanced().buffers().getRebuildTimes().playerMoveTimeout) {
      if (lastUpdatedPos == null
          || Math.abs(newPos.getX()
              - lastUpdatedPos.getX()) > CONFIG.client().advanced().buffers().getRebuildTimes().playerMoveDistance * 16
          || Math.abs(newPos.getZ()
              - lastUpdatedPos.getZ()) > CONFIG.client().advanced().buffers().getRebuildTimes().playerMoveDistance
                  * 16) {
