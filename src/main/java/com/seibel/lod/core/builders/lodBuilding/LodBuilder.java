...
public class LodBuilder {
  ...
  /**
   * This cannot be final! Different world have different height, and in menu,
   * this causes Null Exceptions
   */
  // public static final short MIN_WORLD_HEIGHT =
  // MC.getWrappedClientWorld().getMinHeight();
  ...
  // public static final ExecutorService lodGenThreadPool =
  // Executors.newFixedThreadPool(8, new
  // ThreadFactoryBuilder().setNameFormat("Lod-Builder-%d").build());

  private final ExecutorService lodGenThreadPool = Executors
      .newSingleThreadExecutor(new LodThreadFactory(this.getClass().getSimpleName(), Thread.NORM_PRIORITY - 1));
  ...
  public void generateLodNodeAsync(IChunkWrapper chunk, LodWorld lodWorld, IDimensionTypeWrapper dim,
      DistanceGenerationMode generationMode, boolean override, boolean genAll, Runnable endCallback,
   ...
   */
  public boolean generateLodNodeFromChunk(LodDimension lodDim, IChunkWrapper chunk, LodBuilderConfig config,
      boolean override, boolean genAll) {
    try {
      if (chunk == null)
      ...
      if (!config.quickFillWithVoid) {
        for (int i = 0; i < 16 * 16; i++) {
          ...
          if (!DataPointUtil.doesItExist(data[i * maxVerticalData]))
            throw new RuntimeException("writeVerticalData result: Datapoint does not exist at " + chunk.getMinX() + subX
                + ", " + chunk.getMinZ() + subZ);
          if (DataPointUtil.getGenerationMode(data[i * maxVerticalData]) != config.distanceGenerationMode.complexity)
            throw new RuntimeException("writeVerticalData result: Datapoint invalid at " + chunk.getMinX() + subX + ", "
                + chunk.getMinZ() + subZ);
        }
      } else {
        for (int i = 0; i < 16 * 16; i++) {
      ...
      // This MUST be done after the data is generated, to ensure that during the
      // generation, the data is valid.
      if (!canGenerateLodFromChunk(chunk)) // TODO Why are we calling this again? - James
        return false; // Answer: Because concurrency change may cause the chunk to have invalid
                      // data, like light.

      // ClientApi.logToChat(Level.INFO, "chunk Z: " + chunk.getChunkPosZ());

      if (genAll) {
        return writeAllLodNodeData(lodDim, region, chunk.getChunkPosX(), chunk.getChunkPosZ(), data, config, override);
      } else {
        return writePartialLodNodeData(lodDim, region, chunk.getChunkPosX(), chunk.getChunkPosZ(), data, config,
            override);
      }
    } catch (RuntimeException e) {
  ...
  private boolean writeAllLodNodeData(LodDimension lodDim, LodRegion region, int chunkX, int chunkZ, long[] data,
      LodBuilderConfig config, boolean override) {
    ...
    try {
      ...
      // chunk.getChunkPosX(), chunk.getChunkPosZ(), chunk.getMinX(), chunk.getMinZ(),
  ...
  private boolean writePartialLodNodeData(LodDimension lodDim, LodRegion region, int chunkX, int chunkZ, long[] data,
      LodBuilderConfig config, boolean override) {
    region.isWriting.incrementAndGet();
    try {
      byte targetLevel = region.getMinDetailLevel();
      int vertQual = DetailDistanceUtil.getMaxVerticalData(targetLevel);
      int lodCount = (targetLevel >= LodUtil.CHUNK_DETAIL_LEVEL) ? 1 : 1 << (LodUtil.CHUNK_DETAIL_LEVEL - targetLevel);
      if (targetLevel != 0) {
        ...
        for (int subX = 0; subX < lodCount; subX++) {
          for (int subZ = 0; subZ < lodCount; subZ++) {
            long[] toBeMerged = DataPointUtil.extractDataArray(data, 16, 16, subX * lodWidth, subZ * lodWidth, lodWidth,
                lodWidth);
      ...
      for (int i = 0; i < data.length; i += vertQual) {
        if (!DataPointUtil.doesItExist(data[i])
          ...
          EVENT_LOGGER.error("NULL data at {}, detail {}, vertQual {}, lodCount {}, chunkPos [{},{}]\n" + "Data: {}", i,
              targetLevel, vertQual, lodCount, chunkX, chunkZ, DataPointUtil.toString(data[i]));
      ...
      region.addChunkOfData(targetLevel, LevelPosUtil.convert(LodUtil.CHUNK_DETAIL_LEVEL, chunkX, targetLevel),
          LevelPosUtil.convert(LodUtil.CHUNK_DETAIL_LEVEL, chunkZ, targetLevel), lodCount, lodCount, data, vertQual,
          override);
      region.regenerateLodFromArea(targetLevel, LevelPosUtil.convert(LodUtil.CHUNK_DETAIL_LEVEL, chunkX, targetLevel),
          LevelPosUtil.convert(LodUtil.CHUNK_DETAIL_LEVEL, chunkZ, targetLevel), lodCount, lodCount);

      if (!region.doesDataExist(targetLevel, LevelPosUtil.convert(LodUtil.CHUNK_DETAIL_LEVEL, chunkX, targetLevel),
          LevelPosUtil.convert(LodUtil.CHUNK_DETAIL_LEVEL, chunkZ, targetLevel), config.distanceGenerationMode))
  ...
  private void writeVerticalData(long[] data, int dataOffset, int maxVerticalData, IChunkWrapper chunk,
      LodBuilderConfig config, int chunkSubPosX, int chunkSubPosZ) {

    int totalVerticalData = (chunk.getHeight());
    // int totalVerticalData = 384;
    ...
    while (y >= chunk.getMinBuildHeight()) {
      ...
      dataToMerge[count] = DataPointUtil.createDataPoint(height - chunk.getMinBuildHeight(),
          depth - chunk.getMinBuildHeight(), color, lightSky, lightBlock, generation);
      topBlock = false;
      y = depth - 1;
      count++;
    }
    long[] result = DataPointUtil.mergeMultiData(dataToMerge, totalVerticalData, maxVerticalData);
    if (result.length != maxVerticalData)
      throw new ArrayIndexOutOfBoundsException();
    System.arraycopy(result, 0, data, dataOffset, maxVerticalData);
  }

  public static final LodDirection[] DIRECTIONS = new LodDirection[] { LodDirection.UP, LodDirection.DOWN,
      LodDirection.WEST, LodDirection.EAST, LodDirection.NORTH, LodDirection.SOUTH };
  ...
  /**
   * Find the lowest valid point from the bottom. Used when creating a vertical
   * LOD.
   */
  private int determineBottomPointFrom(IChunkWrapper chunk, LodBuilderConfig builderConfig, int xAbs, int yAbs,
      int zAbs, boolean strictEdge) {
    ...
    if (strictEdge) {
      ...
      if (blockAbove != null && !blockAbove.shouldRender(config.client().worldGenerator().getBlocksToAvoid())) { // The
                                                                                                                 // above
                                                                                                                 // block
                                                                                                                 // is
                                                                                                                 // skipped.
    ...
    for (int y = yAbs - 1; y >= chunk.getMinBuildHeight(); y--) {
      ...
      if (isLayerValidLodPoint(nextBlock)) {
        if (!strictEdge)
          continue;
        if (currentBlockDetail.equals(nextBlock))
          continue;
        if (!hasCliffFace(chunk, xAbs, y, zAbs))
          continue;
      }
      depth = (y + 1);
      break;
  ...
  private int getLightValue(IChunkWrapper chunk, int x, int y, int z, boolean hasCeiling, boolean hasSkyLight,
      boolean topBlock) {
    int skyLight;
    int blockLight;
